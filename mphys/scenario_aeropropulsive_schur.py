...
class ScenarioAeropropulsiveSchur(Scenario):
    ...
    def mphys_make_aeroprop_conn(self, aero2prop_conn, prop2aero_conn):
        ...
        for k, v in aero2prop_conn.items():
            self.connect("coupling.coupling_2.aero.%s" % k, "coupling.coupling_2.prop.%s" % v)
        for k, v in prop2aero_conn.items():
            self.connect("coupling.coupling_2.prop.%s" % k, "coupling.coupling_2.aero.%s" % v)
...
class CouplingAeropropulsive(CouplingGroup):
    ...
    def setup(self):
        ...
        coupling_group = CouplingAeropropulsiveSecondLayer(
            aero_builder=aero_builder, prop_builder=prop_builder, scenario_name=self.name
        )
        self.mphys_add_subsystem("coupling_2", coupling_group)
...
class CouplingAeropropulsiveSecondLayer(CouplingGroup):
    """
    The standard aeropropulsive coupling problem.
    """
